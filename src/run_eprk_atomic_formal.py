...
from formal_execution_authorization import (
    AUTHORIZATION_PATH,
...
EXECUTION_OUTPUT_DIR = OUT / "formal_execution"
CHECKPOINT_DIR = EXECUTION_OUTPUT_DIR / "checkpoints"
EXECUTION_STATE_PATH = EXECUTION_OUTPUT_DIR / "execution_checkpoint.json"
ENVIRONMENT_PATH = EXECUTION_OUTPUT_DIR / "environment.json"
...
_AUTHORIZATION_CACHE: dict[tuple[str, int, int], tuple[dict[str, Any], str]] = {}
...
def verified_authorization(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if cached is not None:
        return cached
    authorization, errors = verify_authorization(resolved)
    if errors:
        raise RuntimeError("formal execution authorization failed: " + "; ".join(errors))
    value = (authorization, sha256_file(resolved))
    _AUTHORIZATION_CACHE.clear()
    _AUTHORIZATION_CACHE[key] = value
    return value
...
def atomic_write_json(path: Path, value: Any) -> None:
    ...
    # Retrying preserves the atomic replace contract without losing a completed run.
...
def verify_frozen_sources() -> None:
    ...
    for name, digest in freeze["source_hashes"].items():
        if sha256_file(resolve_project_path(name)) != digest:
...
def run_solver(solver: str, instance_id: str, seed: int, *, primary_budget: int | None,
    ...
    metrics["profile_hash"] = FORMAL_PROFILE_HASH if solver == "eprk" else sha256_file(resolve_project_path("MRTA_HGA_ATOMIC_CONTROL.py"))
...
def calibration_one(budget: int, instance: str, solver: str, seed: int) -> dict[str, Any]:
    ...
    if target.is_file():
        ...
        if (saved.get("status") == "success" and saved.get("requested_primary_budget") == budget
                ...
                == (FORMAL_PROFILE_HASH if solver == "eprk" else sha256_file(resolve_project_path("MRTA_HGA_ATOMIC_CONTROL.py")))):
...
def preflight() -> dict[str, Any]:
    commands = [
        ...
        [sys.executable, "src/run_unified_experiments.py", "--validate-protocol"],
        [sys.executable, "src/run_unified_experiments.py", "--self-check"],
    ]
    results = []
    ...
    legacy = load_json(resolve_project_path("UNIFIED_EXPERIMENT_PROTOCOL.json"))
...
def _matching_success(base: Path, entry: Mapping[str, Any], authorization_sha256: str) -> Path | None:
    for attempt in sorted(base.glob("attempt_*")):
        result_path = attempt / "result.json"
        result_hash_path = attempt / "result.json.sha256"
        collision_path = attempt / "collision_postprocess.json"
        if result_path.is_file() and result_hash_path.is_file():
            value = load_json(result_path)
            identity_ok = (
                value.get("status") == "success"
                and value.get("resume_key") == entry["resume_key"]
                and value.get("command_hash") == entry["command_hash"]
                and value.get("protocol_hash") == entry["protocol_hash"]
                and value.get("authorization_sha256") == authorization_sha256
                and result_hash_path.read_text(encoding="ascii").strip() == sha256_file(result_path)
            )
            if identity_ok and collision_path.is_file():
                return attempt
    return None


def _quarantine_mismatches(base: Path, entry: Mapping[str, Any], authorization_sha256: str) -> None:
    for attempt in sorted(base.glob("attempt_*")):
        result_path = attempt / "result.json"
        ...
        result_hash_path = attempt / "result.json.sha256"
        mismatch = (
            value.get("resume_key") != entry["resume_key"]
            or value.get("command_hash") != entry["command_hash"]
            or value.get("protocol_hash") != entry["protocol_hash"]
            or value.get("authorization_sha256") != authorization_sha256
            or not result_hash_path.is_file()
            or result_hash_path.read_text(encoding="ascii").strip() != sha256_file(result_path)
        )
        if mismatch:
            target = QUARANTINE_DIR / str(entry["run_id"]) / f"{int(time.time() * 1000)}_{attempt.name}"
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.move(str(attempt), str(target))


def execute_entry(entry: Mapping[str, Any], *, authorization_path: Path = AUTHORIZATION_PATH) -> dict[str, Any]:
    authorization, authorization_sha256 = verified_authorization(authorization_path)
    protocol = load_json(PROTOCOL_PATH)
    if protocol["approval_gates"]["formal_run_approved"] is not False:
        raise RuntimeError("frozen protocol was modified instead of using independent authorization")
    if sha256_file(PROTOCOL_PATH) != entry["protocol_hash"]:
        raise RuntimeError("manifest protocol hash mismatch")
    if int(entry["solver_seed"]) not in authorization["authorized_solver_seeds"]:
        raise RuntimeError("run seed is outside authorization scope")
    if entry["mode"] not in authorization["authorized_modes"]:
        raise RuntimeError("run mode is outside authorization scope")
    if entry["solver"] not in authorization["authorized_solvers"]:
        raise RuntimeError("solver is outside authorization scope")
    base = ROOT / entry["expected_output_path"]
    base.mkdir(parents=True, exist_ok=True)
    existing = _matching_success(base, entry, authorization_sha256)
    if existing:
        return {"run_id": entry["run_id"], "status": "success", "resumed": True, "attempt_path": existing.relative_to(ROOT).as_posix()}
    _quarantine_mismatches(base, entry, authorization_sha256)
        ...
        try:
            ...
            if not success:
                ...
                last = {"run_id": entry["run_id"], "status": "failed", "attempt_path": attempt.relative_to(ROOT).as_posix(),
                        "reason": payload["failure_reason"], "critical_integrity_failure": True}
                continue
            atomic_write_json(result_path, payload)
            ...
            (attempt / "result.json.sha256").write_text(sha256_file(result_path) + "\n", encoding="ascii")
...
RAW_FIELDS = [
    ...
    "route_cache_hits", "generations", "evaluations_per_second",
    "normalized_makespan", "normalized_load_imbalance", "normalized_idle_distance",
    "best_trace_json", "authorization_sha256", "result_sha256",
...
FAILURE_FIELDS = [
    "run_id", "attempt", "status", "exception_type", "exception_message", "stop_reason",
    "stderr", "traceback", "result_path",
]
...
def rebuild_tables() -> dict[str, int]:
    ...
    for entry in manifest["entries"]:
        ...
        for attempt in sorted(base.glob("attempt_*")):
            ...
            for key in RAW_FIELDS:
                if key in metrics:
                    row[key] = metrics[key]
            normalized = metrics.get("normalized_components", {})
            row["normalized_makespan"] = normalized.get("makespan")
            row["normalized_load_imbalance"] = normalized.get("load_imbalance")
            row["normalized_idle_distance"] = normalized.get("idle_distance")
            row["evaluations_per_second"] = (
                float(metrics["primary_evaluations"]) / float(metrics["solver_wall_clock_time_s"])
                if float(metrics.get("solver_wall_clock_time_s", 0.0)) > 0 else None
            )
            row["best_trace_json"] = json.dumps(metrics.get("best_trace", []), ensure_ascii=False, separators=(",", ":"))
            row["authorization_sha256"] = value.get("authorization_sha256")
    ...
    write_csv(EXECUTION_OUTPUT_DIR / "raw_runs.csv", raw_rows, RAW_FIELDS)
    write_csv(EXECUTION_OUTPUT_DIR / "collision_runs.csv", collision_rows, COLLISION_FIELDS)
    write_csv(EXECUTION_OUTPUT_DIR / "run_failures.csv", failures, FAILURE_FIELDS)
    return {"success": len(success_ids), "collision": len(collision_rows), "failed_attempts": len(failures)}


def rebuild_checkpoint_summary() -> None:
    rows = []
    for path in sorted(CHECKPOINT_DIR.glob("checkpoint_*.json")):
        value = load_json(path)
        rows.append({
            "completed_manifest_entries": value["completed_manifest_entries"],
            "success": value["counts"]["success"],
            "collision": value["counts"]["collision"],
            "failed_attempts": value["counts"]["failed_attempts"],
            "last_run_id": value["last_run_id"],
            "created_at": value["created_at"],
            "checkpoint_path": path.relative_to(ROOT).as_posix(),
        })
    fields = [
        "completed_manifest_entries", "success", "collision", "failed_attempts",
        "last_run_id", "created_at", "checkpoint_path",
    ]
    write_csv(EXECUTION_OUTPUT_DIR / "checkpoint_summary.csv", rows, fields)


def run_manifest(max_runs: int | None = None, *, authorization_path: Path = AUTHORIZATION_PATH) -> dict[str, Any]:
    authorization, authorization_sha256 = verified_authorization(authorization_path)
    manifest = load_json(MANIFEST_PATH)
    if (len(manifest.get("entries", [])) != authorization["authorized_run_count"]
            or authorization["authorized_run_count"] != 4680):
        raise RuntimeError("formal manifest length is not exactly 4680")
    current_branch = subprocess.check_output(
        ["git", "branch", "--show-current"], cwd=ROOT, text=True
    ).strip()
    if current_branch != authorization["execution_branch"]:
        raise RuntimeError("current branch is outside formal execution authorization")
    EXECUTION_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    if not ENVIRONMENT_PATH.is_file():
        environment = environment_evidence()
        environment.update({
            "pr_head_sha": authorization["pr_head_sha"],
            "source_branch": authorization["source_branch"],
            "execution_branch": authorization["execution_branch"],
            "authorization_sha256": authorization_sha256,
            "formal_execution_started_at": utc_now(),
        })
    ...
    failed_ids = set(state.get("failed_run_ids", []))
    valid_success_ids: set[str] = set()
    saved_success_ids = set(state.get("success_run_ids", []))
    for entry in manifest["entries"]:
        if entry["run_id"] not in saved_success_ids:
            continue
        base = ROOT / entry["expected_output_path"]
        if _matching_success(base, entry, authorization_sha256) is not None:
            valid_success_ids.add(entry["run_id"])
    processed = valid_success_ids | failed_ids
    state["success_run_ids"] = sorted(valid_success_ids)
    ...
    for entry in manifest["entries"]:
        ...
        outcome = execute_entry(entry, authorization_path=authorization_path)
        ...
        if outcome["status"] == "failed":
            reason = outcome.get("reason", "unknown formal-run failure")
            if state.get("last_failure_reason") == reason:
                state["consecutive_same_root_failures"] = int(state.get("consecutive_same_root_failures", 0)) + 1
            else:
                state["last_failure_reason"] = reason
                state["consecutive_same_root_failures"] = 1
        else:
            state["last_failure_reason"] = None
            state["consecutive_same_root_failures"] = 0
        state["last_run_id"] = entry["run_id"]
        state["updated_at"] = utc_now()
        atomic_write_json(EXECUTION_STATE_PATH, state)
        count += 1
        if outcome.get("critical_integrity_failure"):
            raise RuntimeError(f"formal matrix paused on critical integrity failure: {outcome.get('reason')}")
        if int(state.get("consecutive_same_root_failures", 0)) >= 3:
            raise RuntimeError("formal matrix paused after three consecutive failures with the same root cause")
...
def main() -> None:
    ...
    run_parser.add_argument("--authorization", type=Path, default=AUTHORIZATION_PATH)
    ...
    if args.command == "preflight":
        ...
        protocol_arg = args.protocol if args.protocol.is_absolute() else ROOT / args.protocol
        manifest_arg = args.manifest if args.manifest.is_absolute() else ROOT / args.manifest
        authorization_arg = args.authorization if args.authorization.is_absolute() else ROOT / args.authorization
        if protocol_arg.resolve() != PROTOCOL_PATH.resolve() or manifest_arg.resolve() != MANIFEST_PATH.resolve():
            raise RuntimeError("formal runner accepts only the frozen protocol and manifest paths")
        if not args.resume:
            raise RuntimeError("formal run-manifest requires explicit --resume")
        batch_size = args.batch_size if args.batch_size is not None else args.max_runs
        run_manifest(batch_size, authorization_path=authorization_arg)
    else:
