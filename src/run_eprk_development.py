...
RESULT_ROOT = ROOT / "results" / "eprk"
PROTOCOL = resolve_project_path("EPRK_MA_DEVELOPMENT_PROTOCOL.json")
DEFAULTS = resolve_project_path("eprk_ma_defaults.json")
...
def protected_hashes() -> Dict[str, str]:
    return {name: sha(resolve_project_path(name)) for name in PROTECTED}
...
def load_context(name: str):
    ...
    normalization = json.loads(resolve_project_path("atomic_normalization_spec.json").read_text(encoding="utf-8"))["instances"][name]
...
def race() -> Dict[str, Any]:
    ...
    write_csv(RESULT_ROOT / "eprk_parameter_racing_results.csv", rows)
...
def equal_primary() -> List[Dict[str, Any]]:
    ...
    write_csv(RESULT_ROOT / "eprk_development_runs.csv", rows)
    ...
    write_csv(RESULT_ROOT / "eprk_work_amplification.csv", work)
    return rows
...
def equal_time(primary_rows: Sequence[Mapping[str, Any]]) -> List[Dict[str, Any]]:
    ...
    write_csv(RESULT_ROOT / "eprk_equal_time_runs.csv", rows)
    return rows
...
def create_unrun_outputs() -> None:
    for path in (RESULT_ROOT / "eprk_holdout_runs.csv",):
...
def holdout() -> Dict[str, Any]:
    ...
    development_primary = list(csv.DictReader((RESULT_ROOT / "eprk_development_runs.csv").open(encoding="utf-8-sig")))
    time_rows = []
    for name in ("w30", "w45", "w60"):
    ...
    write_csv(RESULT_ROOT / "eprk_holdout_runs.csv", primary_rows + time_rows)
...
def ablation() -> List[Dict[str, Any]]:
    ...
    write_csv(RESULT_ROOT / "eprk_ablation_runs.csv", rows)
    return rows
