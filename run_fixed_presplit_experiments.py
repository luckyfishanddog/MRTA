...
def preprocessing_outputs() -> None:
    ...
    _write_csv(ROOT / "fixed_presplit_preprocessing_summary.csv", rows)
    boundaries = {}
    ...
    (ROOT / "fixed_presplit_boundary_candidates.json").write_text(
...
def run_one(phase: str, instance: str, variant: str, seed: int, budget: int,
    ...
    command = [str(PYTHON), str(ROOT / "MRTA_ABMA_FIXED_PRESPLIT.py"),
               "--instance", str(ROOT / f"data/instances/fixed_presplit/{instance}_fixed_presplit.json"),
               "--normalization", str(ROOT / "fixed_presplit_normalization.json"),
...
def run_correctness(resume: bool = True) -> List[Dict[str, Any]]:
    ...
    _write_csv(ROOT / "fixed_presplit_equivalence_results.csv", rows)
    return rows
...
def summarize_runtime(values: Sequence[Dict[str, Any]]) -> None:
    ...
    _write_csv(ROOT / "fixed_presplit_runtime_runs.csv", rows)
    amplification = [{key: row[key] for key in (
        ...
        for row in rows]
    _write_csv(ROOT / "fixed_presplit_work_amplification.csv", amplification)
    ...
    (ROOT / "fixed_presplit_runtime_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8"
    )


def run_conditional(resume: bool = True) -> List[Dict[str, Any]]:
    equivalence = list(csv.DictReader((ROOT / "fixed_presplit_equivalence_results.csv").open(encoding="utf-8-sig")))
    ...
    runtime_rows = list(csv.DictReader((ROOT / "fixed_presplit_runtime_runs.csv").open(encoding="utf-8-sig")))
