...
class FixedProtocolTests(unittest.TestCase):
    def test_independent_protocol_and_closed_gates(self):
        new = json.loads(resolve_project_path("UNIFIED_EXPERIMENT_PROTOCOL_FIXED_PRESPLIT_DEV.json").read_text(encoding="utf-8"))
        old = json.loads(resolve_project_path("UNIFIED_EXPERIMENT_PROTOCOL.json").read_text(encoding="utf-8"))
        manifest = json.loads(resolve_project_path("formal_experiment_manifest.json").read_text(encoding="utf-8"))
    ...
    def test_protected_and_instance_hashes(self):
        protocol = json.loads(resolve_project_path("UNIFIED_EXPERIMENT_PROTOCOL_FIXED_PRESPLIT_DEV.json").read_text(encoding="utf-8"))
        for group in ("protected_file_sha256", "fixed_instance_file_sha256",
            ...
            for relative, expected in protocol[group].items():
                self.assertEqual(hashlib.sha256(resolve_project_path(relative).read_bytes()).hexdigest(), expected)
