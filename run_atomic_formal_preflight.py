...
def protected_check() -> tuple[bool, dict[str, Any]]:
...
def perform(*, skip_unit_tests: bool = False, skip_solver_smoke: bool = False) -> dict[str, Any]:
    OUT.mkdir(parents=True, exist_ok=True)
    checks: list[dict[str, Any]] = []
    branch = git("branch", "--show-current")
    status = git("status", "--porcelain")
    checks.append(exact("01_git_worktree_state", branch == "fix/atomic-formal-preflight-normalization-waiver", {
        "branch": branch, "dirty_entry_count": len(status.splitlines()) if status else 0,
        "dirty_allowed_before_commit": True,
    }))

    ok, details = protected_check(); checks.append(exact("02_protected_hashes", ok, details))
    ...
    waiver, waiver_errors = verify_waiver_file(WAIVER, WAIVER_HASH)
    audit = load_json(OUT / "legacy_normalization_field_diff.json")
    ...
    ok, details = companion_hash(FORMAL / "EPRK_ATOMIC_FORMAL_PROTOCOL.json", FORMAL / "EPRK_ATOMIC_FORMAL_PROTOCOL.sha256")
    ...
    protocol = load_json(FORMAL / "EPRK_ATOMIC_FORMAL_PROTOCOL.json")
    ...
    legacy_protocol = load_json(ROOT / "UNIFIED_EXPERIMENT_PROTOCOL.json")
    legacy_errors = legacy_runner.validate_protocol(legacy_protocol, ROOT / "UNIFIED_EXPERIMENT_PROTOCOL.json", OUT / "legacy_validation_tmp")
    ...
    summary = {
        ...
        "full_4680_matrix_executed": False,
        "checks": checks,
    }
...
def main() -> None:
    ...
    summary = perform(skip_unit_tests=args.skip_unit_tests, skip_solver_smoke=args.skip_solver_smoke)
    print(json.dumps({
